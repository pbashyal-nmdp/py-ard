...
from .data_access import ConnectionPool, HLADataRepository
...
class ARD(object):
    """
    ARD reduction for HLA - Refactored with Data Access Layer and Connection Pooling
    """
    ...
    def __init__(
        ...
        # Initialize connection pool and data repository
        self._initialize_data_access_layer(imgt_version, max_connections)
    ...
    def _initialize_data_access_layer(self, imgt_version: str, max_connections: int):
        """Initialize connection pool and data repository"""
        # Get database path
        actual_db_path = db.get_db_path(self._data_dir, imgt_version)
        ...
        self.data_repository = HLADataRepository(self.connection_pool)
    ...
    def _freeze_reference_data(self):
        """Freeze reference data for Python >= 3.9"""
    ...
    def __del__(self):
        """Close connections when ARD instance is destroyed"""
        if hasattr(self, "connection_pool"):
            self.connection_pool.close_all()
        if hasattr(self, "data_repository"):
    ...
    def redux(self, glstring: str, redux_type: VALID_REDUCTION_TYPE = "lgx") -> str:
        """Main redux method using specialized handlers and data access layer"""
    ...
    # Delegate methods to handlers (same as before)
    ...
    def cwd_redux(self, allele_list_gl: str) -> str:
        """CWD reduction using data repository"""
        alleles = []
        ...
        # Use data repository for CWD lookup
        ciwd_for_locus = self.data_repository.get_cwd_alleles(locus)
    ...
    def similar_alleles(self, prefix: str) -> Union[List, None]:
        """Find similar alleles using data repository"""
        if "*" not in prefix:
            return None
        ...
        if fields:
            if len(fields.split(":")) == 2:
                ...
                if mac_prefix.isalpha():
                    similar_mac_names = self.data_repository.similar_mac(mac_prefix)
            ...
            similar_allele_names = self.data_repository.similar_alleles(prefix)
    ...
    def get_db_version(self) -> str:
        """Get database version using data repository"""
        return self.data_repository.get_db_version()

    def refresh_mac_codes(self) -> None:
        """Refresh MAC codes and clear cache"""
